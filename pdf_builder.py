...
def _draw_applicant_signature(c: canvas.Canvas, signature_data: Any, family: str) -> None:
    ...
    boxes = {
        ...
        "more60": (775, 847, 320, 76),
    }
    left_px, top_px, width_px, height_px = boxes.get(family, boxes["regular"])

    box_x = _x(left_px)
    box_y = _y(top_px + height_px)
    box_width = _x(width_px)
    box_height = _x(height_px)
...
def build_form6_pdf(data: Mapping[str, Any], include_back: bool = True) -> bytes:
    ...
    _draw_applicant_signature(c, data.get("applicant_signature"), family)
