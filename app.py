...
def prepare_signature_upload(uploaded_file) -> str:
    ...
    raw = uploaded_file.read(MAX_SIGNATURE_BYTES + 1)
    if not raw:
    ...
    try:
        ...
        raise ValueError("Upload a valid PNG or JPG signature image.")

    if (image.format or "").upper() not in {"PNG", "JPEG", "JPG"}:
...
requests_table = Table(
    ...
    Column("status", String(50), nullable=False, server_default="Submitted to AO II", index=True),
...
def request_pdf(tracking_no: str):
    ...
    # Generated forms must always reflect the latest code and request data.
    # These headers prevent browsers and Vercel's edge cache from reopening
    # an older PDF at the same URL.
    response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    response.headers["Pragma"] = "no-cache"
    response.headers["Expires"] = "0"
    response.headers["X-SPNHS-PDF-LAYOUT"] = "v9"
    return response
...
def admin_request_detail(request_id: int):
    ...
    if request.method == "POST":
        validate_csrf()
        status = request.form.get("status", "Submitted to AO II")
